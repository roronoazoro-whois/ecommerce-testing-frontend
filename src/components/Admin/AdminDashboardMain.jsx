...
const AdminDashboardMain = () => {
  ...
  //Sử dụng redux lấy các giá trị từ slice
  const dispatch = useDispatch();
  //slice order và seller
  const { adminOrders, adminOrderLoading } = useSelector((state) => state.order);
  const { sellers } = useSelector((state) => state.seller);
  // lấy dữ liệu khi render
  useEffect(() => {
  ...
  //Tạo bảng chứa danh sách đơn hàng - order
  const columns = [
  ...
  //mảng chứa dữ liệu cho từng bộ dữ liệu của bảng
  const row = [];
  adminOrders &&
  ...
  return (
    <>
      {
        adminOrderLoading ? (/* kiểm tra load dữ liệu */
          <Loader /> 
        ) : (
